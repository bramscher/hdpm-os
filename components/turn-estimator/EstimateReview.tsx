...
export default function EstimateReview({ estimateId }: { estimateId: string }) {
  ...
  async function act(action: 'APPROVED' | 'DECLINED' | 'schedule' | 'invoice') {
    ...
    try {
      if (action === 'schedule') {
        ...
        router.push(`/maintenance/workspace?job=${job.job_id}&schedule=1`);
  ...
  return <section className="space-y-5">
    ...
    <div className="flex flex-wrap gap-4 text-sm"><Link href="/turn-estimator/price-book" className="text-green-800 underline">Price book</Link><Link href="/maintenance/workspace?view=schedule" className="text-green-800 underline">Availability & planned revenue</Link>{data?.canSchedule&&data.estimate.work_order_id&&data.estimate.status!=='approved'&&<Link className="text-green-800 underline" href={`/maintenance/workspace?work_order=${data.estimate.work_order_id}&schedule=1`}>Plan a visit (scope not approved)</Link>}</div>
