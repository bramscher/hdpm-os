...
export async function GET() {
 ...
 const [staff,policies,audit]=await Promise.all([
  db.from('staff').select('person,email,active,access_role').order('person'),
 ...
 return NextResponse.json({staff:(staff.data||[]).map(s=>{const p=policies.data?.find(p=>p.person===s.person);return {...s,overrides:p?.overrides||{},version:p?.version||0,capabilities:effectiveCapabilities(s.access_role,s.active,p?.overrides||{})};}),audit:audit.data||[]});
}
