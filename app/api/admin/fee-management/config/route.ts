...
import { parseTierRules, parseWeights } from '@/lib/fee-management/model';

/** PUT { tierTargets?, priorityWeights? } — validated, then upserted. */
export async function PUT(request: NextRequest) {
  ...
  if (body.tierTargets !== undefined) {
    const rules = parseTierRules(body.tierTargets);
    if (!rules) return NextResponse.json({ error: 'Tier ranges must be valid and must not overlap' }, { status: 400 });
    rows.push({ org_id: 'hdpm', key: 'tier_targets', value: rules, updated_at: now, updated_by: guard.email });
  }
