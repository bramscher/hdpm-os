...
- **Autonomy ladder per (agent, action_type):** L0 observe/report → L1 draft (human sends) → L2 act-on-tap (button in Slack/SMS) → L3 act-then-notify → L4 silent autonomous. Everything starts at L1 or L2. Promotion rule: ≥4 weeks at current level with <5% edit/reject rate and zero incidents → propose promotion *to Craig*, never self-promote — and never past the Q2 ceiling for that action type.
...
- **Brief B** — agent_proposal / agent_outbox / agent_config migrations + channel adapter interface *(safe to run now)*
