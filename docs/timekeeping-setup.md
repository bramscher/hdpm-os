...
Use **Download review Excel** to regenerate a review copy while correcting payroll inputs. Once the period is ready, **Rebuild Excel · new version** saves another payroll package using current timecards and rules; older versions remain available. **Carrying emergency house phone · stipend** tracks stipend days separately and does not create emergency overtime or require after-hours interval classification.

**Overtime & payroll setup** calculates Sunday–Saturday weekly overtime above 40 worked hours and the company’s 1.5× after-hours emergency premium. Enter emergency work as a separate interval and mark its premium eligibility. Confirm missing opening-workweek hours from prior payroll before creating an export. Everyone defaults to weekly overtime eligibility; confirmed exemptions require an admin’s reason. See [overtime rules, report interpretation and deployment](timekeeping-overtime.md). Wage rates and final dollar pay remain the payroll processor’s responsibility.
