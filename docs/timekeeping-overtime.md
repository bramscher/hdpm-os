...
Craig authorized a company policy on September 16, 2026: after-hours emergency work receives 1.5× even below 40 weekly hours. This includes actual emergency phone work. It is a company premium, not a general Oregon requirement for all night or weekend work. Record the eligible work as a separate interval and select **After-hours emergency work · 1.5×**. The Emergency work day flag requires explicit interval classification before payroll export. A whole regular shift must not be marked just because one emergency occurred that day. Source: [US DOL overtime guidance](https://www.dol.gov/general/topic/wages/overtimepay).

Craig clarified that **carrying the emergency house phone** is paid by a separate stipend. The phone checkbox tracks carrying days only. It does not imply actual emergency work, trigger an interval-classification error, or add worked or overtime hours. Excel shows phone-carrying days separately from emergency-work hours. Payroll must include on-call compensation in the regular rate unless an applicable exclusion permits otherwise; separation into a stipend column does not exclude it from the regular rate. Source: [DOL on-call pay opinion](https://www.dol.gov/sites/dolgov/files/WHD/legacy/files/2008_09_22_06_FLSA.pdf).
...
**Download review Excel** rebuilds a clearly marked review copy from current timecards and current rules, even while payroll inputs or approvals need attention. Missing overtime inputs appear as pending, not zero. This copy is not a saved payroll package and must not be used to pay employees. Once ready, **Rebuild Excel · new version** creates and downloads another saved version from current records. Use it after corrections or calculation updates. **Download saved vN** uses that version’s retained source data.
...
Apply [20260916_timekeeping_phone_stipend.sql](../supabase/migrations/20260916_timekeeping_phone_stipend.sql) after the overtime migration to remove phone-carrying flags from the database’s emergency-work validation. This replaces the validation function without rewriting any payroll records.
