...
import { currentStaffOwner } from '@/lib/staff-lifecycle';
...
export function getPilotConfig(): PilotConfig {
  const recipients = (process.env.AGENT_PILOT_RECIPIENTS ?? '')
    ...
    .filter(Boolean);
  return {
    recipients: [...new Set(recipients.map(currentStaffOwner))],
 ...
 *
 * Craig took over on 2026-09-15. Departed-owner env settings route to Craig.
 * The name must match a `staff.person` row that has an
 * email (for the Outlook draft) and, for the SMS/tap path, a `slack_user_id`.
 *
 *   ESTIMATE_CHASER_OWNER — staff person name. Unset → 'Craig'.
 */
export function getEstimateChaserOwner(): string {
  return currentStaffOwner(process.env.ESTIMATE_CHASER_OWNER?.trim() || 'Craig');
}
